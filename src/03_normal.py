...
var_95_normal = norm.ppf(0.05, mean_return, std_return) # Now calculating Var & ES if log returns were normally distributed 
var_95_normal_dollars = var_95_normal * portfolio_value
var_99_normal = norm.ppf(0.01, mean_return, std_return)
var_99_normal_dollars = var_99_normal * portfolio_value
var_999_normal = norm.ppf(0.005, mean_return, std_return) 
var_999_normal_dollars = var_999_normal * portfolio_value

alpha_95 = 0.05
z_95 = norm.ppf(alpha_95)
es_95_normal = mean_return - std_return * norm.pdf(z_95) / alpha_95
es_95_normal_dollars = es_95_normal * portfolio_value

alpha_99 = 0.01
z_99 = norm.ppf(alpha_99)
es_99_normal = mean_return - std_return * norm.pdf(z_99) / alpha_99
es_99_normal_dollars = es_99_normal * portfolio_value

alpha_999 = 0.005
z_999 = norm.ppf(alpha_999)
es_999_normal = mean_return - std_return * norm.pdf(z_999) / alpha_999
es_999_normal_dollars = es_999_normal * portfolio_value
...
hist_var_999 = np.percentile(port1_log_returns, 0.1)  
hist_es_999 = port1_log_returns[port1_log_returns <= hist_var_999].mean()  
...
print(f"\n95% Normal VaR:                   {var_95_normal:>8.2%}")
print(f"99% Normal VaR:                   {var_99_normal:>8.2%}")
print(f"95% Normal ES:                    {es_95_normal:>8.2%}")
print(f"99% Normal ES:                    {es_99_normal:>8.2%}")
print("\n" + "-" * 60)

print(f"\n{'Metric':<20} {'Normal':>12} {'Historical':>12} {'Difference':>12}\n") # Prints a table with a comparision column 
print(f"{'95% VaR':<20} {var_95_normal:>12.2%} {hist_var_95:>12.2%} {(var_95_normal - hist_var_95):>12.2%}")
print(f"{'99% VaR':<20} {var_99_normal:>12.2%} {hist_var_99:>12.2%} {(var_99_normal - hist_var_99):>12.2%}")
print(f"{'95% ES':<20} {es_95_normal:>12.2%} {hist_es_95:>12.2%} {(es_95_normal - hist_es_95):>12.2%}")
print(f"{'99% ES':<20} {es_99_normal:>12.2%} {hist_es_99:>12.2%} {(es_99_normal - hist_es_99):>12.2%}")
...
plt.axvline(var_95_normal * 100, color = "orange", linestyle = "--", linewidth = 2, label = f"Normal 95% VaR: {var_95_normal:.2%}")
plt.axvline(var_99_normal * 100, color = "red", linestyle = "--", linewidth = 2, label = f"Normal 99% VaR: {var_99_normal:.2%}")
...
plt.axvline(hist_es_95 * 100, color = "darkred", linestyle = "-", linewidth = 2, label = f"Historical 95% ES: {hist_es_95:.2%}")
plt.axvline(hist_es_99 * 100, color = "red", linestyle = "-", linewidth = 2, label = f"Historical 99% ES: {hist_es_99:.2%}")
plt.axvline(es_95_normal * 100, color = "orange", linestyle = "--", linewidth = 2, label = f"Normal 95% ES: {es_95_normal:.2%}")
plt.axvline(es_99_normal * 100, color = "gold", linestyle = "--", linewidth = 2, label = f"Normal 99% ES: {es_99_normal:.2%}")
plt.xlabel("Daily Log Return (%)", fontweight = "bold")
plt.ylabel("Density", fontweight = "bold")
...
historical_values = [hist_var_95 * 100, hist_var_99 * 100, hist_var_999 * 100, hist_es_95 * 100, hist_es_99 * 100, hist_es_999 * 100]
normal_values = [var_95_normal * 100, var_99_normal * 100, var_999_normal * 100, es_95_normal * 100, es_99_normal * 100, es_999_normal * 100]
