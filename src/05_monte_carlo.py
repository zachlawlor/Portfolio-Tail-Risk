...
np.random.seed(42)  # Added for reproducibility since random module is used here 
...
bootstrap_var_95 = equal_weight_results["var_95"] 
...
for name in all_portfolio_log_returns.columns:
    ...
    comparison_results.append({
        "Portfolio": name,
        "95% VaR (Loss)": abs(result["var_95"]),  
        "95% ES (Loss)": abs(result["es_95"]),
        "99% VaR (Loss)": abs(result["var_99"]),
        "99% ES (Loss)": abs(result["es_99"]),
...
print(f"\n95% Bootstrap VaR (Loss):           {abs(bootstrap_var_95):>8.2%}")
print(f"99% Bootstrap VaR (Loss):           {abs(bootstrap_var_99):>8.2%}")
print(f"95% Bootstrap ES (Loss):            {abs(bootstrap_es_95):>8.2%}")
print(f"99% Bootstrap ES (Loss):            {abs(bootstrap_es_99):>8.2%}\n")
print("-" * 60)

print(f"\n{'Metric':<20} {'Historical':>12} {'Normal':>12} {'Student-t':>12} {'Bootstrap':>12}\n")
print(f"{'95% VaR (Loss)':<20} {abs(hist_var_95):>12.2%} {abs(norm_var_95):>12.2%} {abs(t_var_95):>12.2%} {abs(bootstrap_var_95):>12.2%}")
print(f"{'99% VaR (Loss)':<20} {abs(hist_var_99):>12.2%} {abs(norm_var_99):>12.2%} {abs(t_var_99):>12.2%} {abs(bootstrap_var_99):>12.2%}")
print(f"{'95% ES (Loss)':<20} {abs(hist_es_95):>12.2%} {abs(norm_es_95):>12.2%} {abs(t_es_95):>12.2%} {abs(bootstrap_es_95):>12.2%}")
print(f"{'99% ES (Loss)':<20} {abs(hist_es_99):>12.2%} {abs(norm_es_99):>12.2%} {abs(t_es_99):>12.2%} {abs(bootstrap_es_99):>12.2%}")
...
plt.axvline(bootstrap_var_95 * 100, color = "orange", linestyle = "--", linewidth = 2, label = f"95% VaR Loss: {abs(bootstrap_var_95):.2%}")
plt.axvline(bootstrap_var_99 * 100, color = "red", linestyle = "--", linewidth = 2, label = f"99% VaR Loss: {abs(bootstrap_var_99):.2%}")
plt.axvline(bootstrap_es_95 * 100, color = "darkorange", linestyle = "-", linewidth = 2, label = f"95% ES Loss: {abs(bootstrap_es_95):.2%}")
plt.axvline(bootstrap_es_99 * 100, color = "darkred", linestyle = "-", linewidth = 2, label = f"99% ES Loss: {abs(bootstrap_es_99):.2%}")
...
historical_vals = [abs(hist_var_95) * 100, abs(hist_var_99) * 100]
normal_vals = [abs(norm_var_95) * 100, abs(norm_var_99) * 100]
t_vals = [abs(t_var_95) * 100, abs(t_var_99) * 100]
bootstrap_vals = [abs(bootstrap_var_95) * 100, abs(bootstrap_var_99) * 100]
...
metrics = ["95% ES", "99% ES"]
historical_vals = [abs(hist_es_95) * 100, abs(hist_es_99) * 100]
normal_vals = [abs(norm_es_95) * 100, abs(norm_es_99) * 100]
t_vals = [abs(t_es_95) * 100, abs(t_es_99) * 100]
bootstrap_vals = [abs(bootstrap_es_95) * 100, abs(bootstrap_es_99) * 100]
x_pos = np.arange(len(metrics))
width = 0.2
plt.bar(x_pos - 1.5*width, historical_vals, width, label = "Historical", color = "blue", alpha = 0.7, edgecolor = "black")
plt.bar(x_pos - 0.5*width, normal_vals, width, label = "Normal", color = "red", alpha = 0.7, edgecolor = "black")
